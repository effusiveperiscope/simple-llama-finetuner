...
def load_base_model():
    ...
    model = transformers.LlamaForCausalLM.from_pretrained(
        '/mnt/nvme1n1p2/Code/llama_models/llama-7b-hf',
        load_in_8bit=True,
    ...
    def data_generator():
        with open(input_file) as f:
            for l in f:
                if l == "\n":
                    continue
                yield tokenize(to_dict(l.strip()))

    # Use huggingface's internal data loading
    data = datasets.Dataset.from_generator(data_generator)
    data = data.shuffle()
...
def tokenize_and_train(
    ...
    paragraphs = [to_dict(x) for x in  # Assume already stripped
            split_at(training_text, lambda x: x == '\n')]
